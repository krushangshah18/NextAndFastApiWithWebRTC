...
def _resolve_device(device: str, min_vram_gb: float = _MIN_VRAM_GB) -> tuple[str, str]:
    ...
    "auto" logic:
      1. No CUDA driver / GPU → CPU
      2. Free VRAM < min_vram_gb → CPU (prevents OOM / system freeze)
      3. Otherwise → CUDA

    "cuda" / "cpu" → used as-is, with a VRAM warning on cuda if low.
    """
    if device == "cpu":
    ...
    try:
        ...
        if device == "auto" and free_vram < min_vram_gb:
            reason = (
                f"free VRAM {free_vram:.1f} GB < {min_vram_gb:.1f} GB minimum"
            )
...
class ObjectDetector:
    ...
    def __init__(
        ...
        # FP16: only on CUDA — CPU half-precision is slower, not faster
    ...
    def device_info(self) -> dict:
        ...
        if self.device == "cuda" and torch.cuda.is_available():
            ...
            info.update({
                ...
                "gpu_mem_alloc_mb": round(
                    torch.cuda.memory_allocated(0) / 1024 / 1024, 1
                ),
                "gpu_mem_reserved_mb": round(
